...
/// Seed one page (with the given outgoing same-project links) into an explicit
/// scope, so a test can drive the scoped `links` figures that
/// `GET /admin/status?workspace=&project=` reports.
async fn seed_linked_page(
    store: &Store,
    workspace: &str,
    project: &str,
    path: &str,
    ...
    let page = NewPage {
        ...
        links: links
            .iter()
            .map(|target| ai_memory_core::LinkTarget {
                workspace: None,
                project: None,
...
/// A partial scope pair is refused on `/admin/status` too, and the full pair
/// answers with this project's figures (adversarial + control, per `AGENTS.md`
...
async fn status_scoped_links_rejects_partial_scope() {
    ...
    for uri in [
        "/admin/status?workspace=default",
        "/admin/status?project=app",
    ] {
        let resp = app
            .clone()
            .oneshot(
    ...
    // Control: the full pair is answered and carries the scoped object.
    let resp = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/admin/status?workspace=default&project=app")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    let body: serde_json::Value = serde_json::from_slice(&body_bytes(resp).await).unwrap();
    assert_eq!(
        body["links_scope"]["links_from_latest_pages"].as_u64(),
        Some(1),
        "{body}"
    );
    assert_eq!(
        body["links_scope"]["unresolved_links_from_latest_pages"].as_u64(),
        Some(1),
        "{body}"
    );
}

/// An unknown scope is refused and stays refused: the route resolves through
/// the no-create lookup, so the first 404 cannot have minted the scope.
#[tokio::test]
async fn status_scoped_links_fails_closed_on_an_unknown_scope() {
    ...
    for uri in [
        "/admin/status?workspace=default&project=ghost",
        "/admin/status?workspace=ghost&project=app",
    ] {
        let resp = app
            .clone()
            .oneshot(
    ...
    // Adversarial half: a creating path would answer 200 on this retry.
    let resp = app
        .clone()
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/admin/status?workspace=default&project=ghost")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        ...
        "the refused scope must not have been created"
    );

    // Control: the scope that does exist answers.
    let resp = app
        .oneshot(
            Request::builder()
                .method("GET")
...
/// Same-named projects in two workspaces stay isolated in the scoped figures.
#[tokio::test]
async fn status_scoped_links_do_not_leak_across_workspaces() {
    ...
    seed_linked_page(
        ...
        "app",
        "notes/two.md",
        &["notes/missing.md", "notes/also-missing.md"],
    )
    .await;
    let app = admin_router(state);
    ...
    assert_eq!(
        body["links_scope"]["links_from_latest_pages"].as_u64(),
        Some(1),
        "default/app must not include the same-named project in `other`: {body}"
    );
    assert_eq!(
        body["links_scope"]["unresolved_links_from_latest_pages"].as_u64(),
        Some(1),
    ...
    // Store-wide both workspaces count, so the scoped figure is a real subset.
    assert_eq!(
        body["derived"]["links_from_latest_pages"].as_u64(),
        Some(3),
        "{body}"
    );

    // Control: the other workspace's own figures (2 links, both unresolved).
    let resp = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/admin/status?workspace=other&project=app")
                .body(Body::empty())
                .unwrap(),
        )
        .await
