...
| 9 | Scope resolution fail-closed | `ai-memory-store/src/scope.rs` no-create `lookup_existing_*`; create only via `create_explicit_scope`; `ai-memory-mcp/src/admin.rs` `handle_status` resolves an optional `?workspace=&project=` pair through `lookup_ws_proj_no_create` (never `create_ws_proj`), so a scoped status read cannot mint a scope | `scope.rs` no-auto-create + `unscoped_write_with_unresolvable_coordinate_errors`; `admin_status_search.rs` `status_scoped_links_rejects_partial_scope`, `status_scoped_links_fails_closed_on_an_unknown_scope`, `status_scoped_links_do_not_leak_across_workspaces` | STRONG |
